...
static int proc_llenar_archivo(struct seq_file *m, void *v) {
    ...
    seq_printf(m, "<tr> <th>#</th><th>PID</th><th>Nombre</th><th>Usuario</th><th>Estado</th><th>Porcentaje RAM</th></tr>");
    ...
    for_each_process(task){
        ...
        seq_printf(m, "<tr><td>%d</td><td>%d</td><td>%s</td><td>%d</td><td>%s</td><td>%llu</td></tr>", contador_procesos, task->pid, task->comm, task->cred->uid, cadena_estado, size*100/S(i.totalram)); //Agrego un proceso a la tabla
        ...
        list_for_each(list, &task->children){
            ...
            //seq_printf(m, "<tr><td>%d</td><td>%d</td><td>%s</td><td>%d</td><td>%llu</td><td>%d</td><td>%s</td></tr>", contador_procesos, task_child->pid, task_child->comm, task_child->cred->uid, size*100/S(i.totalram), task_child->cpuset_mem_spread_rotor, cadena_estado); //Agrego un proceso a la tabla            
            seq_printf(m, "<tr><td>%d</td><td>%d</td><td>%s</td><td>%d</td><td>%s</td><td>%llu</td></tr>", contador_procesos, task_child->pid, task_child->comm, task_child->cred->uid, cadena_estado, size*100/S(i.totalram)); //Agrego un proceso a la tabla
